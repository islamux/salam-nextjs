...
// Configuration
const DART_LIST_FILE = path.join(__dirname, '../FIX_MODEL_ISSUE/elm_list_2_new_order.dart');
const DART_TEXT_FILE = path.join(__dirname, '../FIX_MODEL_ISSUE/elm_text_ders_two.dart');
const OUTPUT_FILE = path.join(__dirname, '../public/khwater-data-chapter2-detailed.json');
...
function extractArray(body, fieldName, textMappings) {
  ...
  // Match variable references (e.g., ElmTextDersTwo.titleTwoOne_1)
  const varRegex = /ElmTextDersTwo\.(\w+)/g;
  let varMatch;
...
function extractString(body, fieldName, textMappings) {
  const regex = new RegExp(`${fieldName}:\\s*ElmTextDersTwo\\.(\\w+)`, 'g');
  const match = regex.exec(body);
...
function migrate() {
  try {
    ...
    const jsonData = convertToJson(items, '2');
