...
/**
 * Transform Chapter 19 from Dart source to JSON
...
const dartFilePath = path.join(__dirname, 'texts-original/text/elm_text_ders_nineteen.dart');
...
// Write to file
const outputPath = path.join(__dirname, 'public/khwater/19.json');
fs.writeFileSync(outputPath, JSON.stringify(jsonData, null, 2), 'utf8');

console.log('✅ Chapter 19 transformation complete!');
...
console.log(`   Expected Titles: 11 (Got: ${titleCount})`);
console.log(`   Expected Subtitles: 9 (Got: ${subtitleCount})`);
console.log(`   Expected Texts: 44 (Got: ${textCount})`);
console.log(`   Expected Ayahs: 37 (Got: ${ayahCount})`);
console.log(`   Expected Total: 101 (Got: ${items.length})`);

if (items.length === 101 && titleCount === 11) {
