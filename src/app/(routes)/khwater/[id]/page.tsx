// Dynamic Khwater chapter page with code splitting
...
const ContentRenderer = dynamic(
  () => import('@/components/khwater/ContentRenderer'),
  {
...
const ShareButton = dynamic(
  () => import('@/components/khwater/ShareButton'),
  {
...
// Generate static paths for all chapters (SSG with ISR)
...
export async function generateMetadata({ params }: PageProps) {
  ...
  // Create a description from the first few items
  const description =
    chapter.slice(0, 3).map((item) => item.texts?.[0]).filter(Boolean).join(' • ') ||
    `محتوى الفصل ${id} من كتاب خواطر - عن الدين والحياة  `;

  return {
    title: `الفصل ${id} - كتاب خواطر`,
    description: `اقرأ الفصل ${id} من كتاب خواطر. ${description}`,
    keywords: `كتاب خواطر, فصل ${id}, نصوص إسلامية, الدين والحياة`,
    openGraph: {
      title: `الفصل ${id} - كتاب خواطر`,
      ...
      tags: ['Islam', 'Spiritual', 'Arabic', 'Book'],
    },
    twitter: {
...
export default async function KhwaterChapterPage({ params }: PageProps) {
  ...
  return (
    ...
    dangerouslySetInnerHTML={{
      __html: JSON.stringify({
        ...
        keywords: 'كتاب خواطر, نصوص إسلامية, الدين والحياة',
      }),
    }}
    />
    ...
    <h1 className="arabic-title text-4xl font-bold mb-4">
    الفصل {id}
    </h1>
    </header>
  ...
  >
    ...
    {hasPrevious ? (
      ...
      aria-label={`انتقل إلى الفصل ${currentId - 1}`}
    >
      <svg
    ...
    {hasNext ? (
      <Link
      href={`/khwater/${currentId + 1}`}
      className="inline-flex items-center px-6 py-3 bg-blue-600 text-white hover:bg-blue-700 rounded-lg transition-colors"
      aria-label={`انتقل إلى الفصل ${currentId + 1}`}
    >
      الفصل التالي
      <svg
