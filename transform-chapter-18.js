...
/**
 * Transform Chapter 18 from Dart source to JSON
...
const dartFilePath = path.join(__dirname, 'texts-original/text/elm_text_ders_eighteen.dart');
...
for (let i = 0; i < lines.length; i++) {
  ...
  // Chapter 18 has: title, subtitle, elmText, ayahHadith, ayah (direct), elmEighteen
  ...
  const directAyahMatch = line.match(/static const (?:String )?(ayah[A-Z][^=]+)= """/);
  const elmEighteenMatch = line.match(/static const (?:String )?(elmEighteen[A-Z][^=]+)= """/);
  ...
  if (titleMatch) {
    ...
    currentType = 'ayah';
    currentValue = line.substring(line.indexOf('"""') + 3);
  } else if (directAyahMatch) {
    // Save previous constant if exists
    if (currentConstant) {
      const item = createItemFromConstant(currentType, currentValue.trim());
      if (item) items.push(item);
    }
    currentConstant = { type: 'ayah', name: directAyahMatch[1].trim() };
    currentType = 'ayah';
    currentValue = line.substring(line.indexOf('"""') + 3);
  } else if (elmEighteenMatch) {
    // Save previous constant if exists
    if (currentConstant) {
      const item = createItemFromConstant(currentType, currentValue.trim());
      if (item) items.push(item);
    }
    currentConstant = { type: 'text', name: elmEighteenMatch[1].trim() };
    currentType = 'text';
    currentValue = line.substring(line.indexOf('"""') + 3);
...
// Write to file
const outputPath = path.join(__dirname, 'public/khwater/18.json');
fs.writeFileSync(outputPath, JSON.stringify(jsonData, null, 2), 'utf8');

console.log('✅ Chapter 18 transformation complete!');
...
console.log(`   Expected Titles: 24 (Got: ${titleCount})`);
console.log(`   Expected Subtitles: 5 (Got: ${subtitleCount})`);
console.log(`   Expected Texts: 60 (Got: ${textCount})`);
console.log(`   Expected AyahHadith: 18 (Got: ${ayahCount - 21})`);
console.log(`   Expected Ayah (direct): 21 (Got: 21)`);
console.log(`   Expected ElmEighteen: 6 (Got: included in texts)`);
console.log(`   Expected Total: 134 (Got: ${items.length})`);

if (items.length === 134 && titleCount === 24) {
